...
function CleanFilesInLabStorageAccount($DevTestLabName, $ImagesToSave, $goldenImagesFolder, $goldenImageFiles)
{
    ...
    if($thingsToDelete -and $thingsToDelete.Count -gt 0)
    {
        ...
        $storageContext = New-AzStorageContext -StorageAccountName $storageAcctName -StorageAccountKey $labStorageInfo.storageAcctKey
        ...
        foreach($thingToDelete in $thingsToDelete)
        {
            ...
            Remove-AzStorageBlob -Context $storageContext -Container $rootContainerName -Blob $vhdBlobName -Force

            $jsonBlobName = $vhdBlobName.Replace('.vhd', '.json')
            Write-Output "  Deleting $jsonBlobName"
            Remove-AzStorageBlob -Context $storageContext -Container $rootContainerName -Blob $jsonBlobName -Force
        }
    }
    else
    {
...
foreach ($selectedLab in $sortedLabList){
    ...
    $allImages = Get-AzResource -ResourceName $selectedLab.LabName -ResourceGroupName $selectedLabRG -ResourceType 'Microsoft.DevTestLab/labs/customImages' -ApiVersion '2016-05-15'
    # Get the images to delete (generated by factory + only old images for each group based on retension policy)
    $imageObjectsToDelete = $allImages | Where-Object {$_.Tags } | ForEach-Object { New-Object -TypeName PSObject -Prop @{
